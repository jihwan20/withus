...
public class DateData {
	...
	public Map<String, Integer> today_info(DateData dateData) {
		...
		int startDay = calendar.getMinimum(java.util.Calendar.DATE);
		int endDay = calendar.getActualMaximum(java.util.Calendar.DAY_OF_MONTH);
		...
		// 날짜 관련
		System.out.println("[BJH(search_month)] : " + search_month);
		// 캘린더 함수 end
	...
	// 스케줄 사용시 사용될 사용자
	public DateData(String year, String month, String date, String value, Schedule[] schedule_date_arr) {
		if((month != null && month != "") && (date != null && date != "")) {
			this.year = year;
			this.month = month;
			this.date = date;
			this.value = value;
			this.schedule_data_arr = schedule_date_arr;
		}
	}
